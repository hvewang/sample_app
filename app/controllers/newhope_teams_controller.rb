class NewhopeTeamsController < ApplicationController
  ...
  def all
	@title = "New Hope Team"
	@newhope_teams = NewhopeTeam.find(:all, :conditions=>['status=?', 'Active'], :order=>"sort_order, updated_at desc").paginate(:page => params[:page], :per_page => 2)
  ...
  def advisor
	...
	@newhope_teams = NewhopeTeam.find(:all, :conditions=>['record_type=? and status=?', 'Advisor', 'Active'], :order=>'sort_order, updated_at desc').paginate(:page => params[:page], :per_page => 3)
  end
  ...
  def team
	...
	@newhope_teams = NewhopeTeam.find(:all, :conditions=>['branch_name_en=? and status=?', branch_name, 'Active'], :order=>"sort_order, updated_at desc").paginate(:page => params[:page], :per_page => 3)
