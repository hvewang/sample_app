require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by the Rails when you ran the scaffold generator.

describe ServiceRequestsController do

  def mock_service_request(stubs={})
    @mock_service_request ||= mock_model(ServiceRequest, stubs).as_null_object
  end

  describe "GET index" do
    it "assigns all service_requests as @service_requests" do
      ServiceRequest.stub(:all) { [mock_service_request] }
      get :index
      assigns(:service_requests).should eq([mock_service_request])
    end
  end

  describe "GET show" do
    it "assigns the requested service_request as @service_request" do
      ServiceRequest.stub(:find).with("37") { mock_service_request }
      get :show, :id => "37"
      assigns(:service_request).should be(mock_service_request)
    end
  end

  describe "GET new" do
    it "assigns a new service_request as @service_request" do
      ServiceRequest.stub(:new) { mock_service_request }
      get :new
      assigns(:service_request).should be(mock_service_request)
    end
  end

  describe "GET edit" do
    it "assigns the requested service_request as @service_request" do
      ServiceRequest.stub(:find).with("37") { mock_service_request }
      get :edit, :id => "37"
      assigns(:service_request).should be(mock_service_request)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "assigns a newly created service_request as @service_request" do
        ServiceRequest.stub(:new).with({'these' => 'params'}) { mock_service_request(:save => true) }
        post :create, :service_request => {'these' => 'params'}
        assigns(:service_request).should be(mock_service_request)
      end

      it "redirects to the created service_request" do
        ServiceRequest.stub(:new) { mock_service_request(:save => true) }
        post :create, :service_request => {}
        response.should redirect_to(service_request_url(mock_service_request))
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved service_request as @service_request" do
        ServiceRequest.stub(:new).with({'these' => 'params'}) { mock_service_request(:save => false) }
        post :create, :service_request => {'these' => 'params'}
        assigns(:service_request).should be(mock_service_request)
      end

      it "re-renders the 'new' template" do
        ServiceRequest.stub(:new) { mock_service_request(:save => false) }
        post :create, :service_request => {}
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested service_request" do
        ServiceRequest.stub(:find).with("37") { mock_service_request }
        mock_service_request.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :service_request => {'these' => 'params'}
      end

      it "assigns the requested service_request as @service_request" do
        ServiceRequest.stub(:find) { mock_service_request(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:service_request).should be(mock_service_request)
      end

      it "redirects to the service_request" do
        ServiceRequest.stub(:find) { mock_service_request(:update_attributes => true) }
        put :update, :id => "1"
        response.should redirect_to(service_request_url(mock_service_request))
      end
    end

    describe "with invalid params" do
      it "assigns the service_request as @service_request" do
        ServiceRequest.stub(:find) { mock_service_request(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:service_request).should be(mock_service_request)
      end

      it "re-renders the 'edit' template" do
        ServiceRequest.stub(:find) { mock_service_request(:update_attributes => false) }
        put :update, :id => "1"
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested service_request" do
      ServiceRequest.stub(:find).with("37") { mock_service_request }
      mock_service_request.should_receive(:destroy)
      delete :destroy, :id => "37"
    end

    it "redirects to the service_requests list" do
      ServiceRequest.stub(:find) { mock_service_request }
      delete :destroy, :id => "1"
      response.should redirect_to(service_requests_url)
    end
  end

end
